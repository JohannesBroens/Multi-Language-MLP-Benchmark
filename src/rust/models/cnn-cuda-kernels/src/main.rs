...
struct CudnnConvFwdAlgoPerf {
    algo: i32, status: i32, time: f32, memory: usize,
    determinism: i32, math_type: i32, reserved: [i32; 3],
}
...
impl CudnnState {
    fn new() -> Self {
        ...
        unsafe {
            ...
            cudnn_check(cudnnSetConvolutionMathType(desc_conv1_conv, 2), "setMathType conv1");
...
impl GpuCnn {
    ...
    fn forward_batch(&mut self, d_inputs: *const f32, bs: i32,
        ...
        unsafe {
            ...
            // FC layers (custom tiled matmul -- NO cuBLAS)
            launch_matmul_nn(pool2_out, self.fc1_w, fc1_out, bs, FC1_OUT, FLAT_SIZE);
            launch_bias_relu(fc1_out, self.fc1_b, bs * FC1_OUT, FC1_OUT);

            launch_matmul_nn(fc1_out, self.fc2_w, fc2_out, bs, FC2_OUT, FC1_OUT);
            launch_bias_relu(fc2_out, self.fc2_b, bs * FC2_OUT, FC2_OUT);

            launch_matmul_nn(fc2_out, self.out_w, out, bs, NUM_CLASSES, FC2_OUT);
            launch_bias_softmax(out, self.out_b, bs, NUM_CLASSES);
        }
    }
        ...
        for epoch in 0..num_epochs {
            ...
            for batch in 0..num_batches {
                ...
                unsafe {
                    ...
                    // FC backward (custom tiled matmul -- NO cuBLAS)
                    launch_matmul_tn(fc2_out, d_out, g_out_w, FC2_OUT, NUM_CLASSES, bs);
                    launch_col_sum(d_out, g_out_b, bs, NUM_CLASSES);

                    launch_matmul_nt(d_out, self.out_w, d_fc2, bs, FC2_OUT, NUM_CLASSES);
                    launch_relu_mask(d_fc2, fc2_out, bs * FC2_OUT);

                    launch_matmul_tn(fc1_out, d_fc2, g_fc2_w, FC1_OUT, FC2_OUT, bs);
                    launch_col_sum(d_fc2, g_fc2_b, bs, FC2_OUT);

                    launch_matmul_nt(d_fc2, self.fc2_w, d_fc1, bs, FC1_OUT, FC2_OUT);
                    launch_relu_mask(d_fc1, fc1_out, bs * FC1_OUT);

                    launch_matmul_tn(pool2_out, d_fc1, g_fc1_w, FLAT_SIZE, FC1_OUT, bs);
                    launch_col_sum(d_fc1, g_fc1_b, bs, FC1_OUT);

                    launch_matmul_nt(d_fc1, self.fc1_w, d_flat, bs, FLAT_SIZE, FC1_OUT);
